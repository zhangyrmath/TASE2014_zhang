    ...
    * {\small  Yixiang Chen: The Communication author}
    ...
    As introduced before, we defined CCSL as the specification language in the domain of STeC (see Def.~\ref{def_clock_stec} and Der.~\ref{sat}). Next we specify the properties of 'Railroad Crossing System' using CCSL.
    ...
    \centerline{the gate shall close in at most 20 seconds.}

    \vspace{0.5cm}
    It contains a time expression '20 seconds'. We use the operator '\textbf{DiscretizeBy}' in CCSL to generate a new discrete-time clock $c_{time}$ that ticks every 10s based on an '\textbf{IdealClock}'. The expression of \textbf{Property 2} is given as follows:

    \vspace{0.5cm}
    \centerline{$c_{appr} \prec c_{close} \prec c_{appr\_delay}$;}

    \centerline{$c_{appr\_delay} = c_{appr}$ \textbf{delay} $20$  \textbf{on} $c_{time}$;}
    ...
    \ifx
     ...
     \begin{center}
    $c_{time} =$ \textbf{IdealClock} \textbf{DiscretizeBy} $1$s
     \end{center}

    The formula above indicates that clock $c_{time}$ 'ticks' every 1s (\textbf{IdealClock} can be understood as 'physical clock'), and clock $c_{appr\_delay}$ is defined as a clock being fired after 20 'ticks' of $c_{time}$ when $c_{appr}$ 'ticks'. Fig.~\ref{prop2_1st} shows the timed automata of $c_{appr\_delay}$. One defect of it is that it contains too many states, thinking that if we change '20 seconds' into '400 seconds', the resulted TA would contain about 400 states, that is quite large. One alternative way of enhancing it is to let $c_{time}$ 'discretized' by a larger number, say 10s for example, so the delayed counts for $c_{appr\_delay}$ becomes smaller. We have the following expressions:

    \begin{center}
    $c_{appr} \prec c_{close} \prec c_{appr\_delay}$;

    $c_{appr\_delay} = c_{appr}$ \textbf{delay} $2$  \textbf{on} $c_{time}$;

    $c_{time} =$ \textbf{IdealClock} \textbf{DiscretizeBy} $10$s

    \end{center}

    Fig.~\ref{prop2} shows its corresponding TA. It contains much less states but it is less 'accurate' than the former one.
    This is because if $c_{appr}$ 'ticks' between two ticks of $c_{time}$, the duration until the next tick of $c_{appr\_delay}$ is actually less than duration between 2 ticks of $c_{time}$. Fig.~\ref{prop2_delay_on} shows one possibility, the duration of first tick of $c_{appr}$ and $c_{appr\_delay}$ is less than 20s, while the second duration is equal to 20s.

    \begin{figure}[htp]
    \fontsize{20pt}{25pt}\selectfont
    \scalebox{0.625}{\input{./inkscape_graph/delay_on.pdf_tex}}
    \caption{One condition that the duration between $c_{appr}$ and $c_{appr\_delay}$ is less than 20s}
    \label{prop2_delay_on}
    \end{figure}

    Now let us give a definition of \textbf{IdealClock} by defining the predicate $P$ (Section \uppercase\expandafter{\romannumeral2} gives a definition of \textbf{IdealClock}, see ~\cite{andre08} for more details).
    ...
    \begin{center}
        $\bff{AG} (\bff{AF} $ 'clock ticks' $)$,
    \end{center}

    where 'clock ticks' (in UPPAAL) could be any boolean values indicating that if any CCSL clocks tick. $\bff{AG}$' and '$\bff{AF}$' are the operators in CTL. $A_{CCSL}$ is the automata of CCSL property. '$\times$' is the Decare product over TAs. This CTL formula means that 'clock ticks' happens infinitely often, in other words, there is no such a condition under which 'clock ticks' never happens since some point of time.
    ...
    shows the automata of \textbf{Property 1}. Note that we add $\epsilon$-transitions on each state to allow CCSL clock to tick on no condition. For more details about the encoding from CCSL to TA, refer to \cite{suryadevara13}.
    ...
    \begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=2cm,
      ...
      \node[rec node,draw=none] (4) [right of=3]{$...$};
        ...
        (1) edge [loop above] node {$\epsilon$} (1)
        (2) edge [loop above] node {$\epsilon$} (2)
        (3) edge [loop above] node {$\epsilon$} (3)
        ;
    ...
    \begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=2cm,
      ...
      \node[initial,main node] (1) [] {$0$};
        ...
        (1) edge [loop below] node {$\epsilon$} (1)
        ;
    ...
    \begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=2cm,
      ...
      \node[initial,main node] (1) [] {$0$};
      \node[main node] (2) [right of=1] {$1$};
      \node[main node] (3) [right of=2, xshift=0.25cm] {$2$};
    ...
    In the UPPAAL TA of \textbf{Property 1}, we define each event as boolean variable. In order to synchronize with the Train and Gate agent, we also need to add a function for each event to trigger the boolean variable while the agent triggers the event. Note that in Fig.~\ref{uppaal_prop1}, there is a loop on each state since that we allow each CCSL clock to tick with none boolean variables mimicking the $\epsilon$-transitions in TA shown in Fig.~\ref{prop1_ta}.
    'x$<=$ub' is to make sure that each transition will eventually be triggered.

    Fig.~\ref{3sys} in Appendix A shows a possible running state diagram of system where three agents run concurrently.

    \ifx
    Note that in the UPPAAL TA of \textbf{Property 1} (Fig.~\ref{uppaal_prop1}), we omit the event 'mayPass' since no such action exists in Train system.
    \fi

    \begin{figure}[htp]
    \centering
    \includegraphics[width=10cm]{graph/CCSL_prop1.png}
    \caption{The UPPAAL TA of \textbf{Property 1}}
    \label{uppaal_prop1}
    \end{figure}

    \ifx
    In UPPAAL, the nested statement with $\bff{AG}$ and $\bff{EF}$ is not allowed, which means that we can not proof the liveness of CCSL clock by using the CTL statement '$\bff{AG} (\bff{EF} s)$'. Since the TA of \textbf{Property 1} in our example is a 'circle', so in fact we only need to guarantee the satisfaction of one of states, and thus other states's liveness is guaranteed. On the other side, not checking the liveness of CCSL clock alone, we instead check the occurrence of clock '$c_{close}$' each time after Gate is 'closed'. So we check the liveness of CCSL clock in our example by the following CTL statement on UPPAAL:
    \fi

    To verify the property we combine the TA of Train-Gate system and the TA of property in UPPAAL and check the liveness of each CCSL clock using CTL formula '\textbf{AG} \textbf{AF} "clock ticks" '. To check \textbf{Property 1} we need to check the following CTLs equivalently:

    \begin{enumerate}[1.]
        \item \textbf{AG} \textbf{AF} ('$c_{close}$ ticks')
        \item \textbf{AG} \textbf{AF} ('$c_{open}$ ticks')
        \item \textbf{AG} \textbf{AF} ('$c_{mayPass}$ ticks')
        \item \textbf{AG} ('train pass' $\rightarrow$ \textbf{AF} '$c_{pass}$ ticks')
    \end{enumerate}

    In the last statement, we need to consider the pass of the train agent since $c_{pass}$ never ticks if there is no train passing the crossing. Expression 4 means that whenever if the train passes, the clock '$c_{pass}$' will eventually tick. Fig.~\ref{veriprop1} in Appendix A shows a result of the verification of \textbf{Property 1}.
    ...
    \begin{enumerate}[1.]
        ...
        \item $c_{appr\_delay} = c_{appr}$ \textbf{delay} $2$ \textbf{on} $c_{time}$
        \item $c_{time} =$ \textbf{IdealClock} \textbf{DiscretizeBy} $10$ s
    \end{enumerate}
    \end{center}


    In \cite{suryadevara13}, the encoding from the expression '\textbf{delay on}', '\textbf{DiscretizeBy}' and '\textbf{IdealClock}' into TA is illustrated in detail. Here we only give the transformed TAs. Fig.~\ref{prop2_3} and Fig.~\ref{prop2_4} show the corresponding TA of expression 3 and 4 given above. Fig.~\ref{prop2} gives the TA of \textbf{Property 2}. The TAs of expression 1 and 2 are similar to Fig.~\ref{prop1_1} and Fig.~\ref{prop1_2}, which are omitted here.
    ...
    \begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=3cm,
      ...
      \node[initial,main node] (1) [] {$x\le 10$};
      \node[main node] (2) [right of=1] {$x\le 10$};
      \node[main node] (3) [right of=2] {$x\le 10$};
      %\node [main node] (4) [below of=2] {$x\le 10$};
      %\node [main node] (5) [below of=3] {$x\le 10$};
        ...
        (1) edge [bend right] node {$\langle appr,c\rangle$} (2)
        %(2) edge [bend left] node {$b=999$ ; $\langle appr\_delay, c\rangle$} (1)
        (1) edge [loop, out=130, in=155, looseness=10] node [above] {$c$} (1)
        (1) edge [loop above] node {$\epsilon$} (1)
        %(2.south) edge [bend left=60] node {$b=999$ ; $\langle appr\_delay, c, appr\rangle$} (1.south)
        (2) edge [loop above] node {$\epsilon$} (2)
        %(2) edge [loop right] node {$appr$} (2)
        (2) edge [bend right] node {$c$} (3)
        (2) edge [bend left] node {$\langle appr, c\rangle$} (3)
        (3) edge [loop above] node {$\epsilon$} (3)
        (3.south) edge [bend left] node [above] {$\langle appr\_delay, appr, c\rangle$} (1.south)
        (3.south) edge [bend left=60] node {$\langle appr\_delay, c\rangle$} (1.south)
        (3) edge [loop, out=55, in=30,looseness=10] node [xshift=-0.25cm] {$appr$} (3)
        ;
    \end{tikzpicture}
    \caption{The TA of $c_{appr\_delay} = c_{appr}$ \textbf{delay} $2$ \textbf{on} $c_{time}$}
    \label{prop2_3}
    \end{figure}
    ...
    \begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=4cm,
      ...
      \node[initial,main node] (1) [] {$x\le 10$};
        ...
        \path[every node/.style={font=\sffamily\tiny}]
        (1) edge [loop right] node {$c$ ; $x==10$} (1)

        ;

    \end{tikzpicture}
    \caption{The TA of $c_{time} =$ \textbf{IdealClock} \textbf{DiscretizeBy} $10$ s}
    \label{prop2_4}
    \end{figure}
    ...
    \begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=3cm,
      ...
      \node [main node] (5) [below of=3] {$x\le 10$};
        ...
        (3) edge node {$close$} (5)
        (3) edge [loop above] node {$\epsilon$} (3)
        (4) edge [loop, out=60,in=35,looseness=10] node [above] {$\epsilon$} (4)
        (5) edge [loop, out=130,in=155,looseness=10] node [above] {$\epsilon$} (5)
        (5.south) edge [bend left=70] node {$\langle appr\_delay, c\rangle$} (1.south)
        ;
    \end{tikzpicture}
    ...
    In practice, we encoded \textbf{Property 2} into UPPAAL TA. Fig.~\ref{uppaal_prop2} shows its corresponding UPPAAL TA. We check the CTL expressions as follows for the liveness of CCSL clocks:

    \begin{enumerate}[1.]
        \item \textbf{AG} \textbf{AF} ('$c_{appr}$ ticks')
        \item \textbf{AG} \textbf{AF} ('$c_{close}$ ticks')
        \item \textbf{AG} \textbf{AF} ('$c_{appr\_delay}$ ticks')
    \end{enumerate}

    %It means that whenever train approaches, the clock '$c_{close}$' will eventually tick.

    We omit the result of verification of \textbf{Property 2}, it is similar to the result of \textbf{Property 1}.

    \begin{figure}[htp]
    \centering
    \includegraphics[width=10cm]{graph/CCSL_prop2.png}
    ...
    \appendices
    \section{Additional Figures Pertaining to UPPAAL Verification}
\begin{figure*}[htp]
\centering
...
\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=3cm,
  ...
  \node[initial,main node] (1) [] {$x\le 10$};
  \node[main node] (2) [right of=1] {$x\le 10$};
  \node[main node] (3) [right of=2,draw=none] {$...$};
  \node[main node] (4) [right of=3] {$x\le 10$};
  \node[main node] (5) [above of=3,yshift=-1.5cm,draw=none] {20 trans};
  %\node [main node] (4) [below of=2] {$x\le 10$};
  %\node [main node] (5) [below of=3] {$x\le 10$};
    ...
    (1) edge [loop above] node {$\epsilon$} (1)
    %(2.south) edge [bend left=60] node {$b=999$ ; $\langle appr\_delay, c, appr\rangle$} (1.south)
    (2) edge [loop above,out=130,in=100,looseness=10] node {$\epsilon$} (2)
    %(2) edge [loop right] node {$appr$} (2)
    (2) edge [bend right] node {$c$} (3)
    (2) edge [bend left] node [above] {$\langle appr, c\rangle$} (3)
    (4) edge [loop above,out=130,in=100,looseness=10] node {$\epsilon$} (4)
    (4.south) edge [bend left=15] node [above] {$\langle appr\_delay, appr, c\rangle$} (1.south)
    (4.south) edge [bend left=30] node {$\langle appr\_delay, c\rangle$} (1.south)
    (4) edge [loop, out=55, in=30,looseness=10] node [xshift=-0.25cm] {$appr$} (4)
    (3) edge [bend right] node {$c$} (4)
    (3) edge [bend left] node [above] {$\langle appr, c\rangle$} (4)
    
    ;
    \draw [dashed] (2.north) -- (3cm,1.5cm) node [midway, left] {};
    \draw [dashed] (4.north) -- (9cm,1.5cm) node [midway, left] {};
    \draw  (5.east) -- (9cm,1.5cm) node [midway, left] {};
    \draw  (5.west) -- (3cm,1.5cm) node [midway, left] {};
\end{tikzpicture}
\caption{The TA of $c_{appr\_delay} = c_{appr}$ \textbf{delay} $20$ \textbf{on} $c_{time}$}
\label{prop2_1st}
\end{figure*}
